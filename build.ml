...
let () =
  ...
  let mkcmo name =
    ...
    if name = "main"
    then
      let flags = "-g -w A-7-6-4 -I +lablGL -thread -pp 'sed -f " ^
        (Filename.concat dirname "pp.sed") ^ "'"
      in
      cmopp ~flags ~dirname name
    else cmopp ~flags:"-g -w A-7-6-4 -I +lablGL -thread" ~dirname name;
    (name ^ ".cmo")
  in
